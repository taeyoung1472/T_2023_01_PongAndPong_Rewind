...
// 드로잉 프레임워크용 UIElements를 사용하여 SettingsProvider를 등록
static class MyCustomSettingsUIElementsRegister {
    [SettingsProvider]
    public static SettingsProvider CreateMyCustomSettingsProvider() {
        // 첫 번째 매개변수는 설정 창의 경로
        // 두 번째 매개변수는 이 설정의 범위. 프로젝트 범위의 설정 창에만 나타남
        var provider = new SettingsProvider("Project/MyCustomUIElementsSettings", SettingsScope.Project) {
            label = "BehaviourTree",
            // activateHandler는 사용자가 설정 창에서 설정 항목을 클릭할 때 호출
            activateHandler = (searchContext, rootElement) => {
                var settings = BehaviourTreeSettings.GetSerializedSettings();

                // rootElement는 VisualElement. 자식을 추가하면 OnGUI 기능이
                // SettingsProvider가 UIElements 드로잉 프레임워크를 사용하기 때문에 호출되지 않음(솔직히 먼소린지 잘 모르겠음)
                var title = new Label() {
